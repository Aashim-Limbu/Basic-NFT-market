...
contract ReactionNftTest is Test {
    ...
    //     );
    // }
    function testUserCanToggleReaction() public {
        vm.prank(user);
        reactionNft.mintNft();
        vm.prank(user);
        reactionNft.toggleReaction(0);
        assert(
            reactionNft.getCurrentReaction(0) == ReactionNft.Reaction.UNLIKE
        );
    }
}
